...
const idleTimeoutMs = Number.parseInt(
  process.env.ROCKETSOLVER_SUPERVISOR_IDLE_TIMEOUT_MS ?? String(3 * 60 * 1000),
  10
);
...
const stdoutLogPath = path.join(outputDirectory, 'batch-resume.log');
const stderrLogPath = path.join(outputDirectory, 'batch-resume-error.log');
...
function activityModifiedAt() {
  ...
  for (const filePath of [heartbeatPath, checkpointPath]) {
    try {
      ...
      // The heartbeat is created by the first supervised batch process.
    }
  }
  return latest;
}
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (targets.some((target) => target.type === 'page' && target.webSocketDebuggerUrl)) {
          await delay(5000);
          const stableResponse = await fetch(endpoint, { signal: AbortSignal.timeout(3000) });
          if (stableResponse.ok) {
            const stableTargets = await stableResponse.json();
            if (stableTargets.some((target) => target.type === 'page' && target.webSocketDebuggerUrl)) return;
          }
        }
      }
    } catch {
...
async function startRun() {
  ...
  try {
    activeBatch = spawn(process.execPath, ['scripts/gto/run-rocketsolver-flop-batch.mjs'], {
      cwd: process.cwd(),
      env: {
        ...process.env,
        ROCKETSOLVER_CDP: endpoint,
        ROCKETSOLVER_FLOP_BATCH_OUTPUT: outputDirectory
      },
    ...
    timer = setInterval(() => {
      ...
      try {
        ...
        if (completed > latestCompleted) {
          ...
          log('progress', {
            completed,
            total: checkpoint?.requestedFlops ?? checkpoint?.totalCanonicalFlops ?? null,
...
while (!stopping) {
  ...
  try {
    ...
    const total = checkpoint?.requestedFlops ?? checkpoint?.totalCanonicalFlops ?? 0;
